...
import { getEventDate, getEventDateValue, isUpcomingEvent, sortByEventDate } from "@/lib/event-utils";
...
function MiniCalendar({ events, covers }: { events: EventItem[]; covers: Record<number, string> }) {
    ...
    return (
      ...
      >
        <div className="flex items-center justify-between">
          <span className={["font-semibold text-[11px] sm:text-xs", isToday ? "text-blue-700" : "text-slate-900"].join(" ")}>
            {format(day, "d")}
          </span>
        ...
        <div className="hidden sm:block">
          {dayEvents.slice(0, 2).map((ev) => (
            <div key={ev.id} className="mt-1 truncate rounded bg-blue-100 px-1.5 py-0.5 text-[10px] text-blue-800" title={ev.title}>
              {clampText(ev.title, 14)}
            </div>
          ))}
          {dayEvents.length > 2 && (
            <div className="mt-0.5 text-[9px] text-slate-400">+{dayEvents.length - 2}</div>
          )}
        </div>
      </button>
    );
      ...
      <div className="flex flex-col gap-4 lg:grid lg:grid-cols-3 lg:gap-5">
        ...
        <Card className="overflow-hidden border border-slate-200 bg-white/95 shadow-sm lg:col-span-2">
            ...
            <div className="flex flex-col gap-2 sm:flex-row sm:items-start sm:justify-between">
              ...
              <div className="flex flex-wrap items-center gap-1.5">
                <Button
                  ...
                  onClick={() => { const today = new Date(); setCalMonth(today); setSelectedDate(today); setShowPanel(false); }}
                ...
                <Button variant="ghost" size="icon"
                  onClick={() => mode === "MONTH" ? setCalMonth(subMonths(calMonth, 1)) : setSelectedDate(addDays(selectedDate, -7))}
                  aria-label="Oldingi" className="text-slate-900 h-7 w-7"
                >
                ...
                <div className="text-center min-w-[120px] sm:min-w-[150px]">
                  ...
                  >
                    {mode === "MONTH" ? t("navbar.weekView") : t("navbar.monthView")}
                    <ChevronUp className={["h-3 w-3 transition", mode === "MONTH" ? "rotate-180" : ""].join(" ")} />
                  </button>
                </div>

                <Button variant="ghost" size="icon"
                  onClick={() => mode === "MONTH" ? setCalMonth(addMonths(calMonth, 1)) : setSelectedDate(addDays(selectedDate, 7))}
                  aria-label="Keyingi" className="text-slate-900 h-7 w-7"
                >
          ...
          <CardContent className="bg-white px-2 pb-3 sm:px-4 sm:pb-4">
            ...
            <div className="mb-1 grid grid-cols-7 gap-1">
              {WEEKDAYS.map((day) => (
                <div key={day} className="text-center text-[10px] sm:text-xs font-semibold text-slate-500">
                  {day}
                </div>
              ))}
            </div>
            ...
            {mode === "MONTH" ? (
              <div className="grid grid-cols-7 gap-1">
                {Array.from({ length: emptyCount }).map((_, i) => <div key={`e-${i}`} />)}
              ...
              <div className="mt-3 sm:hidden">
                <div className="mb-1.5 flex items-center justify-between">
                  <span className="text-xs font-bold text-slate-800">
                    {format(selectedDate, "dd MMMM")} — {selectedDayEvents.length} ta tadbir
                  </span>
                  <button
                    type="button"
                    ...
                    return (
                      ...
                      >
                        <div className="h-10 w-10 shrink-0 overflow-hidden rounded-lg border border-slate-100 bg-slate-100">
                          {cover
                            ? <img src={fileUrl(cover)} alt={ev.title} className="h-full w-full object-cover" loading="lazy" />
        ...
        <Card className="hidden border border-slate-200 bg-white/95 shadow-sm lg:block">
          <CardHeader className="bg-white pb-2 pt-4 px-5">
            <CardTitle className="text-base text-slate-900">{t("navbar.reminder")}</CardTitle>
            <p className="text-xs text-slate-500">{format(selectedDate, "dd MMMM yyyy")}</p>
          </CardHeader>
          <CardContent className="max-h-[500px] space-y-2.5 overflow-auto bg-white px-4 pb-4 pr-1">
            {selectedDayEvents.length === 0 ? (
              <div className="text-sm text-slate-500">{t("navbar.noEventsOnDay")}</div>
            ) : (
              selectedDayEvents.map((ev) => {
                  ...
                  >
                    <div className="flex gap-3 p-3">
                      <div className="h-12 w-12 shrink-0 overflow-hidden rounded-xl border border-slate-200 bg-slate-100">
                        {cover
                          ? <img src={fileUrl(cover)} alt={ev.title} className="block h-full w-full object-cover" loading="lazy" />
                          : <div className="h-full w-full bg-gradient-to-br from-blue-100 to-sky-100" />
                        }
                      </div>
                      <div className="min-w-0 flex-1">
                        <div className="flex items-start justify-between gap-2">
                          ...
                          {visibility
                            ? <Badge variant="secondary" className="whitespace-nowrap text-[10px]">{visibility}</Badge>
                            : null
                          }
                        </div>
                        {ev.organizationName
                          ? <div className="mt-2 text-[11px] text-slate-400">{ev.organizationName}</div>
                          : null
                        }
                      </div>
                    </div>
...
export default function Navbar() {
  ...
  const apiUrl = (path: string) => `${ORIGIN}${API_PREFIX}${path.startsWith("/") ? "" : "/"}${path}`;
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        const upcoming = sortByEventDate(list.filter((event) => isUpcomingEvent(event))).slice(0, 120);
        if (!alive) return;
        setEvents(upcoming);

        const results = await Promise.all(upcoming.map(async (event) => {
  ...
  return (
    <>
      <header className="sticky top-0 z-50 border-b border-slate-200/70 bg-white/85 backdrop-blur-xl">
        <div className="mx-auto flex h-16 w-full max-w-7xl items-center justify-between gap-3 px-4 sm:px-6 lg:px-8">
          <Link to="/" className="flex items-center gap-3 text-decoration-none">
            <img src="/png.png" alt="TripDay" className="h-12 w-auto rounded-xl object-contain" />
          </Link>

          <nav className="hidden items-center gap-2 md:flex">
            <NavLink to="/" className={navLinkClass}>
              <span className="inline-flex items-center gap-2"><Home className="h-4 w-4" />{t("common.home")}</span>
            </NavLink>
            <NavLink to="/events" className={navLinkClass}>
              <span className="inline-flex items-center gap-2"><CalendarDays className="h-4 w-4" />{t("common.events")}</span>
            </NavLink>
                ...
                {(hasRole("ADMIN") || hasRole("SUPER_ADMIN")) && (
                  <NavLink to={adminPath} className={navLinkClass}>
                    <span className="inline-flex items-center gap-2">
                      ...
                      {hasRole("SUPER_ADMIN") ? t("common.superAdmin") : t("common.admin")}
                    </span>
                  </NavLink>
                )}
                {hasRole("TOUR_ORGANIZATION") && (
                  <NavLink to="/dashboard" className={navLinkClass}>
                    <span className="inline-flex items-center gap-2"><Building2 className="h-4 w-4" />{t("common.dashboard")}</span>
                  </NavLink>
                )}
                <NavLink to="/profile" className={navLinkClass}>
                  <span className="inline-flex items-center gap-2"><User className="h-4 w-4" />{t("common.profile")}</span>
                </NavLink>
                <button
                  onClick={() => { logout(); navigate("/"); }}
                  className="rounded-xl px-3 py-2 text-sm font-semibold text-slate-700 transition hover:bg-red-50 hover:text-red-600"
                >
                  <span className="inline-flex items-center gap-2"><LogOut className="h-4 w-4" />{t("common.logout")}</span>
                </button>
              </>
            ) : (
              <NavLink to="/login" className="rounded-xl bg-blue-600 px-4 py-2 text-sm font-semibold text-white transition hover:bg-blue-700">
                <span className="inline-flex items-center gap-2"><LogIn className="h-4 w-4" />{t("common.login")}</span>
              </NavLink>
            )}
          </nav>
      ...
      {mobileOpen && (
        <div className="fixed inset-0 z-40 bg-slate-950/35 backdrop-blur-[2px] md:hidden" onClick={close} />
      )}
      ...
      <aside className={[
        ...
        <div className="mb-5 flex items-center justify-between">
          <img src="/png.png" alt="TripDay" className="h-11 w-auto rounded-lg object-contain" />
          <button type="button" onClick={close} className="inline-flex h-10 w-10 items-center justify-center rounded-xl border border-slate-200 text-slate-700">
            <X className="h-5 w-5" />
          </button>
        </div>
        ...
        <div className="flex flex-col gap-1">
          <NavLink to="/" onClick={close} className={mobileLinkClass}><Home className="h-5 w-5" />{t("common.home")}</NavLink>
          <NavLink to="/events" onClick={close} className={mobileLinkClass}><CalendarDays className="h-5 w-5" />{t("common.events")}</NavLink>

          <Dialog>
            <DialogTrigger asChild>
              <button type="button" className="flex w-full items-center gap-3 rounded-xl px-4 py-3 text-sm font-semibold text-slate-800 transition hover:bg-blue-50">
          ...
          {user ? (
            <>
              {(hasRole("ADMIN") || hasRole("SUPER_ADMIN")) && (
                <NavLink to={adminPath} onClick={close} className={mobileLinkClass}>
                  <LayoutDashboard className="h-5 w-5" />
                  {hasRole("SUPER_ADMIN") ? t("common.superAdmin") : t("common.admin")}
                </NavLink>
              )}
              {hasRole("TOUR_ORGANIZATION") && (
                <NavLink to="/dashboard" onClick={close} className={mobileLinkClass}><Building2 className="h-5 w-5" />{t("common.dashboard")}</NavLink>
              )}
              <NavLink to="/profile" onClick={close} className={mobileLinkClass}><User className="h-5 w-5" />{t("common.profile")}</NavLink>
              <button
            ...
            <NavLink to="/login" onClick={close} className="mt-2 flex items-center justify-center gap-3 rounded-xl bg-blue-600 px-4 py-3 text-sm font-semibold text-white transition hover:bg-blue-700">
